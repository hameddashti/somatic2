...
gc()
#==========Create Matrix=======================
chr6_del_cnv<-matrix(0, nrow = ln, ncol = 6)
#==========Fill matrix=========================
for (i in 1:ln) {
  chr6_del_cnv[i,1]<-i+st-1
}
...
if (nrow(case_Del) != 0) {
  for (i in 1:nrow(case_Del)) {
    ...
    k1 <- k1 - st + 1
    k2 <- k2 - st + 1
    chr6_del_cnv[k1:k2, 2] <- chr6_del_cnv[k1:k2, 2] + 1
  }
}
...
if(nrow(control_Del)!=0){
  for (i in 1:nrow(control_Del)){
    ...
    k1<-k1-st+1
    k2<-k2-st+1
    chr6_del_cnv[k1:k2,3]<-chr6_del_cnv[k1:k2,3]+1
  }
}
chr6_del_cnv<-subset(chr6_del_cnv,chr6_del_cnv[,2]>=10)
if(nrow(chr6_del_cnv)!=0){
  for (i in 1:nrow(chr6_del_cnv)){
    m<-matrix(c(chr6_del_cnv[i,2],chr6_del_cnv[i,3],no_case_Del-chr6_del_cnv[i,2],no_control_Del -chr6_del_cnv[i,3]),nrow = 2)
    chr6_del_cnv[i,6] <-fisher.test(m,alternative = "two.sided",conf.level =0.9)$p.value
    chr6_del_cnv[i,4] <-fisher.test(m,alternative = "greater",conf.level =0.9)$p.value
    chr6_del_cnv[i,5] <-fisher.test(m,alternative = "less",conf.level =0.9)$p.value
  }
}
path<-'/home/san/halinejad/Desktop/Dashti/somatic2'
path<-paste0(path,'/Result/chr6_del.csv')
write.csv(chr6_del_cnv,path)
#======================Significant regions=======================
significant_pval<- 1.2e-8
significant_file_Del<- as.data.frame(subset(chr6_del_cnv,chr6_del_cnv[,4]<=significant_pval))
significant_regions_chr6_del_cnv<- significant_file_Del%>%
  as.data.frame() %>%
  mutate(
...
path<-'/home/san/halinejad/Desktop/Dashti/somatic2'
path<-paste0(path,'/Result/regions_chr6_del.csv')
write.csv(significant_regions_chr6_del_cnv,path)