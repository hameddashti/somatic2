...
gc()
#==========Create Matrix=======================
chr16_dup_cnv<-matrix(0, nrow = ln, ncol = 6)
#==========Fill matrix=========================
for (i in 1:ln) {
  chr16_dup_cnv[i,1]<-i+st-1
}
...
if (nrow(case_Dup) != 0) {
  for (i in 1:nrow(case_Dup)) {
    ...
    k1 <- k1 - st + 1
    k2 <- k2 - st + 1
    chr16_dup_cnv[k1:k2, 2] <- chr16_dup_cnv[k1:k2, 2] + 1
  }
}
...
if(nrow(control_Dup)!=0){
  for (i in 1:nrow(control_Dup)){
    ...
    k1<-k1-st+1
    k2<-k2-st+1
    chr16_dup_cnv[k1:k2,3]<-chr16_dup_cnv[k1:k2,3]+1
  }
}
chr16_dup_cnv<-subset(chr16_dup_cnv,chr16_dup_cnv[,2]>=10)
if(nrow(chr16_dup_cnv)!=0){
  for (i in 1:nrow(chr16_dup_cnv)){
    m<-matrix(c(chr16_dup_cnv[i,2],chr16_dup_cnv[i,3],no_case_Dup-chr16_dup_cnv[i,2],no_control_Dup -chr16_dup_cnv[i,3]),nrow = 2)
    chr16_dup_cnv[i,6] <-fisher.test(m,alternative = "two.sided",conf.level =0.9)$p.value
    chr16_dup_cnv[i,4] <-fisher.test(m,alternative = "greater",conf.level =0.9)$p.value
    chr16_dup_cnv[i,5] <-fisher.test(m,alternative = "less",conf.level =0.9)$p.value
  }
}
path<-'/home/san/halinejad/Desktop/Dashti/somatic2'
path<-paste0(path,'/Result/chr16_dup.csv')
write.csv(chr16_dup_cnv,path)
#======================Significant regions=======================
significant_pval<- 1.2e-8
significant_file_Dup<- as.data.frame(subset(chr16_dup_cnv,chr16_dup_cnv[,4]<=significant_pval))
significant_regions_chr16_dup_cnv<- significant_file_Dup%>%
  as.data.frame() %>%
  mutate(
...
path<-'/home/san/halinejad/Desktop/Dashti/somatic2'
path<-paste0(path,'/Result/regions_chr16_dup.csv')
write.csv(significant_regions_chr16_dup_cnv,path)